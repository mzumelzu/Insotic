...
class institucion{
	...
	function getInstitucion(){
		$sql = "select * from insotic_institucion";
		$this->conectar();
		$r = mysql_query ($sql);
		if (mysql_num_rows($r)==0) {
			echo "<script>alert('No tiene resultados la query de instituciones')</script>"; 
			die();
		} 
		else {
			return $r;
	...
	function getInstitucionById($id){
		$sql = "select * from insotic_institucion where id='$id'";
		$this->conectar();
		$r = mysql_query ($sql);
		if (mysql_num_rows($r)==0) {
			echo "<script>alert('No tiene resultados la query de instituciones')</script>"; 
